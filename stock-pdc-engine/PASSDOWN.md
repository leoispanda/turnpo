...
| DAILY_TOP10 冻结事实 `daily/facts.py` `daily/sources.py` | 完成 | 14 | Claude 08-19（加 fact 字段索引） |
| DAILY_TOP10 三轮契约 `daily/contracts.py` | 完成 | 31 | Claude 08-19（D3 引用契约重做） |
| DAILY_TOP10 三轮执行 `daily/{discovery,detail,review}.py` | 完成 | 21 | Claude 08-19（D3 packet 重排） |
...
| DAILY_TOP10 编排与降级 `daily/orchestrator.py` | 完成 | 22 | Claude 08-19 |
| CLI `scripts/pdc_daily_top10.py` | 完成（`plan`/`run`/`show`） | 手测 | Claude 08-19 |
| **真实模型端到端验收** | 已跑 1 次：R1/R2 成功，R3 契约拒绝后安全回退 R2；拒绝原因已修，**待下次真跑验证** | 实跑 | Codex 08-19 |
| 行业映射（行业集中度所需） | 新浪申万一级，5546/5546 全 A 覆盖，默认启用 | 4 | Codex 08-19 |
| 定时调度 | 未做（Codex 负责） | — | — |

离线测试总数：**320 个，全部通过**（其中 DAILY_TOP10 新增 162 个）。
...
### 2026-08-19 · Claude（修 D3 契约 + 复核 Codex 的 Q1–Q7）
- 动了什么：只改终审轮的**引用契约**，没有碰任何策略常量、没有放宽校验。
  1. **引用改成枚举对象**。`fact_ids: ["600968.SH.pivot55"]` 变成
     `fact_refs: [{"ticker": "600968.SH", "field": "pivot55"}]`，ticker 枚举是本轮 20 支终选、
     field 枚举是 17 个字段。Sol 那处 `600968.SZ` 的后缀手滑在新 schema 下**写不出来**，
     而不是写出来再被拒——它一条手滑赔掉了另外 25 条完全正确的修订。
  2. **允许跨 ticker 引用**。Claude 那三条不是幻觉：它在给 `000651.SZ` 的 risk 打分时
     引用了 `600919.SH` 的 ATR、给 `601728.SH` 引用了 `601998.SH` 的动量，做的是同业对比。
     终审的工作就是给 20 支排序，"它的波动率是上一名的两倍"是证据，而且两个数字本来就在
     同一个 packet 里。"只能引本 ticker"是我自己加的，原规格只说"只能引用已有 fact_id"。
     现在引用可以指向本轮任一终选股，跨 ticker 的会记进 `cross_ticker_refs` 留痕。
     仍然引用不存在的事实 → 照样整轮拒绝。
  3. **事实与分数同框**。每支的冻结事实、可引字段、自己的分、匿名对手的分现在在同一个
     block 里，不再要求模型在三个相隔很远的段落之间对照。顺带把重复 20 遍的字段名换成
     `unavailableFields`（通常为空），终审 prompt 从 37.2k 降到 33.1k 字符。
  4. 兼容：席位若无视 schema 仍吐旧的点分字符串，解析器照读（`A.rsi14` 或裸 `rsi14`），
     但含义照旧严格——解析不出真实事实就拒。
- 测试：**320 个全部通过**（+11）。新增 `RealRunRegressionTest`，直接锁住 2026-08-19
  这两种真实死法；另加一条端到端测试，让假席位真的发一条跨 ticker 修订，验证它能一路
  改到 `d3-final-*.json` 并留痕。**没有跑真实模型。**
- 复核 Codex 的 Q1–Q7（用 `daily-20260819-real-01` 的真实数据核过，请 Leo 拍板）：

  | Q | Codex 建议 | Claude 复核 | 真实数据 |
  | --- | --- | --- | --- |
  | Q1 敞口 | 姿态未设按 `neutral=0.9` | **不同意**。市场档位已经压到 0.7，再乘 0.9 = 63%，等于对"没有信息"收两道税。没设姿态的诚实编码是"不调整" | 08-19 实跑敞口 0.7，投 70% |
  | Q2 止损上限 | 12% → 10% | **同意**，但不是免费的 | 10 支里 `601919.SH 中远海控` 10.55%，改 10% 它就出局，第 2 名换人 |
  | Q3 换手缓冲 | 3 → 5 | **两个数都没有证据**。缓冲 5 意味着第 15 名保座、第 11 名进不来，这个倒挂很宽。建议先留 3 并逐日记录 11–15 名的占用情况，跑够 10 个交易日再定 | 今天无持仓，churn 尚无观测 |
  | Q4 行业上限 | 3 → 2 | **同意**。3 席=21% 单一行业，对"分散清单"确实偏高 | 今天银行正好 3 席，改 2 就要挤掉上海银行（第 5 名） |
  | Q5 分歧阈值 | 1.5 → 1.0 | **强烈反对**。1.0 落在分歧的**正常区间中部**，不是尾部 | 46 支的分歧分布 0.635–1.55；>1.5 只有 4 支，>1.0 有 **27 支（59%）**。改 1.0 会踢掉今天 10 席里的 4 席，且替补也多半超标——结果是大量 CASH，而这是被"两个模型日常就差 1 分"触发的，不是被市场触发的 |
  | Q6 Remove 前置 | 维持不前置 | **同意**，无异议 | — |
  | Q7 main_reason | 别用自报 confidence，两席 note 并列 | **同意**。今天两席耗时差 3 倍（223s vs 629s），自报置信度更不该当裁判。CSV 单元格记得截断 | — |

- 关于 Codex 发现的两个 SELL/PAUSE 语义问题（我没改，同意等 Leo 口径）：
  - **`decision` 不参与门槛**确实是个洞。建议：任一席位终审后 `decision == SELL` → 该股不可
    新买（软门槛，持仓降为 PAUSE）；两席都 SELL → 硬门槛，让出席位。
  - **PAUSE 仍显示仓位**：我认为这是对的——PAUSE 是"已持有、不加仓"，目标权重本来就还在。
