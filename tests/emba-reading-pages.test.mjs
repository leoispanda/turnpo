...
assert.ok(berkReading?.editionBridge, "Berk should map the available fourth-edition summary to Day 1");
assert.equal(berkReading.editionBridge.items.length, 6, "Berk bridge should distinguish available and missing topic areas");
assert.ok(berkReading.editionBridge.items.some((item) => item.status === "missing"), "Berk bridge should not pretend the old summary covers everything");
