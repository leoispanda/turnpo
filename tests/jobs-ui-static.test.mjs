...
assert.ok(indexHtml.includes("Follow-ups to process into personal Markdown"));
assert.ok(indexHtml.includes("Generate modified resume"));
assert.ok(indexHtml.includes("Print / Save PDF"));
assert.ok(indexHtml.includes('class="footer-maintenance owner-only"'));
assert.ok(indexHtml.includes('class="ghost-btn footer-maintenance-link" id="openJobsModule"'));
assert.equal(indexHtml.includes('class="profile-function-btn" id="openJobsPage"'), false);
assert.equal(indexHtml.includes('class="gold-btn" id="openJobsModule"'), false);
...
assert.ok(scriptJs.includes("function absorbFollowUpsIntoMarkdown("));
assert.ok(scriptJs.includes("Absorbed into personal Markdown"));
assert.ok(scriptJs.includes("Ready to absorb into personal Markdown"));
...
assert.ok(scriptJs.includes('$("#openJobsPage")?.addEventListener'));
assert.ok(scriptJs.includes('$("#openJobsModule")?.addEventListener'));
assert.ok(scriptJs.includes('$("#analyzeSelectedJob")?.addEventListener'));
