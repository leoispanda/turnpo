...
assert.ok(decisionJs.includes('if (runStrip) runStrip.hidden = !(state.running || state.testing || state.error || run);'));
assert.ok(decisionJs.includes('if (copy) copy.hidden = !(state.running || state.testing || state.error || state.run);'));
