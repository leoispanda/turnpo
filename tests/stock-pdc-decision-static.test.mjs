...
assert.ok(decisionHtml.includes('id="refreshMarketData"'));
assert.ok(demoHtml.includes('id="refreshMarketData"'));
...
assert.ok(decisionJs.includes("MANUAL_REFRESH_GITHUB_REJECTED"));
...
assert.ok(decisionJs.includes("模型分批复核未产生新的有效股票记录"));
...
assert.ok(decisionJs.includes('const expectedSchema = "stock-pdc-hawkeye-v2"'));
assert.ok(decisionJs.includes("Hawkeye Radar snapshot predates full-market accounting"));
assert.ok(decisionJs.includes("Hawkeye Radar market-universe accounting is incomplete"));
assert.ok(decisionJs.includes("Hawkeye Radar market-data coverage is below its required completion threshold"));
assert.ok(decisionJs.includes("Hawkeye Radar does not identify one verified full-market data provider"));
assert.ok(decisionJs.includes("state.refreshingMarketData"));
assert.ok(decisionJs.includes('state.error ? "FAILED"'));
...
assert.ok(stockFunction.includes("const PDC_MEMBER_STAGE_STORAGE = \"PDC_MEMBER_STAGE_V1\""));
assert.ok(stockFunction.includes("function decisionMemberStageKey"));
assert.ok(stockFunction.includes("export async function stockPdcWorkflowScoreMemberBatch"));
assert.ok(stockFunction.includes("export async function stockPdcWorkflowFinalizeMemberStage"));
...
assert.ok(decisionJs.includes("async function api(path, options = {})"));
assert.ok(!decisionJs.includes("controller.abort(), timeoutMs"));
...
assert.ok(stockFunction.includes("Hawkeye Radar does not identify one verified full-market data provider"));
assert.ok(stockFunction.includes("Hawkeye Radar market-data coverage is below its required completion threshold"));
...
assert.ok(stockFunction.includes("MANUAL_REFRESH_GITHUB_REJECTED"));
...
assert.ok(stockFunction.includes("const PDC_REVIEW_BATCH_SIZE = 30;"));
assert.ok(stockFunction.includes("function mergeCompletedReviewBatch"));
assert.ok(stockFunction.includes("function deterministicSecretaryMetrics"));
assert.ok(stockFunction.includes('model: "PDC_SCORECARD_V1"'));
assert.ok(stockFunction.includes("fixed-format scorecard record"));
...
assert.ok(workflowWorker.includes("async function scoreCommitteeStage"));
assert.ok(workflowWorker.includes("await Promise.all(members.map((member) => step.do("));
assert.ok(workflowWorker.includes("stockPdcWorkflowScoreMemberBatch"));
assert.ok(workflowWorker.includes("stockPdcWorkflowFinalizeMemberStage"));
