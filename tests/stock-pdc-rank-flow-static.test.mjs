...
assert.ok(stockHtml.includes("<title>股票大作手 最多 20 名 | Turnpo</title>"));
...
assert.ok(!stockHtml.includes('id="stockAccessGate"'));
...
assert.ok(!stockJs.includes("STOCK_PASSWORD"));
assert.ok(stockJs.includes("renderDataQualityNotice"));
...
assert.ok(syncScript.includes(".filter((fileInfo) => isTradingWeekday(fileInfo.date))"));
...
assert.ok(rankFlow.days.every((day) => {
