---
id: emba-2026-07-learning-index
title: EMBA Monthly Learning Index - July 2026
slug: 2026-07-emba-learning-index
type: monthly_index
program: EMBA
school: Maastricht University
course: Intro Programme
module: July 2026
date: 2026-07-09
year: 2026
month: 2026-07
source_type: monthly_index
source_file:
visibility: private
status: active
tags:
  - leadership
  - learning
  - coaching
  - ai
  - inclusion
  - organizational-learning
keywords:
  - Lead in Learning
  - Leading in Learning Programme
  - memory making
  - listening
  - psychological safety
  - 70/20/10
  - Nonaka Takeuchi
summary: July 2026 captures the first EMBA learning thread: leadership as a practice of better listening, reflection, psychological safety, organizational learning, inclusion, and responsible AI use.
related_topics:
  - leadership
  - ai-and-digital-transformation
  - decision-making
  - personal-growth
rag_include: true
created_at: 2026-07-09
updated_at: 2026-07-10
---

# EMBA Monthly Learning Index - July 2026

## 1. Month Overview

July 2026 starts the EMBA learning system with a practical leadership thread. The strongest move is from "leader as answer giver" toward "leader as builder of better thinking conditions." The notes connect leadership, coaching, listening, reflection, inclusion, AI use, and organizational learning.

## 1A. Searchable Summary

July 2026 is best retrieved for questions about Lead in Learning, LDT, team building, MBA administration, problem-based learning, alumni, leadership posture, listening, coaching, memory making, psychological safety, AI-assisted learning, organizational knowledge creation, inclusion, culture mapping, and storytelling.

The official programme PDF should be used for schedule, room, speaker, location, travel, and session-overview questions. The handwritten-note mirrors should be used for Leo's learning interpretation and reflections.

## 2. Main Courses And Sessions

- Lead in Learning
- MBA Welcome and Need-to-knows
- Team Building and Alumni Forum
- Leadership Development Trajectory
- Memory Making
- Listening and Coaching
- AI in learning and work
- Learning and Development in Organisations
- Inclusion and Diversity
- Storytelling and communication

## 3. Files Added This Month

### Converted Markdown

- [MaastrichtMBA Leading In Learning Programme - July 2026](./converted-md/source-documents/2026-07-leading-in-learning-programme.md)
- [Leadership Development Trajectory - Micole Smits](./converted-md/source-documents/2026-07-leadership-development-trajectory.md)
- [Team Building - Diana Mingo Berdun](./converted-md/source-documents/2026-07-team-building.md)
- [Welcome MBA - Ron Jacobs](./converted-md/source-documents/2026-07-welcome-mba.md)
- [Alumni Forum - Jeroen Duijsinx](./converted-md/source-documents/2026-07-alumni-forum.md)
- [MBA Need To Knows - Jesca Rijpkema](./converted-md/source-documents/2026-07-mba-need-to-knows.md)
- [EMBA July 2026 Handwritten Notes Analysis](./converted-md/2026-07-01-leadership-learning-handwritten-notes.md)
- Per-image Markdown mirrors:
  - [IMG 6900 - Lead in Learning](./converted-md/image-notes/IMG_6900-lead-in-learning.md)
  - [IMG 6901 - Four Levels Of Listening](./converted-md/image-notes/IMG_6901-listening-habits.md)
  - [IMG 6902 - Listening Resistance, Feedback, And Emotion](./converted-md/image-notes/IMG_6902-three-voices.md)
  - [IMG 6903 - Coaching Through Listening And Trust](./converted-md/image-notes/IMG_6903-coaching-listening.md)
  - [IMG 6904 - Memory Making And Reflection](./converted-md/image-notes/IMG_6904-memory-making-reflection.md)
  - [IMG 6905 - True Leadership And Department Value](./converted-md/image-notes/IMG_6905-perspective-and-truth.md)
  - [IMG 6906 - Change, Inclusion, And Blind Spots](./converted-md/image-notes/IMG_6906-organizational-learning-change.md)
  - [IMG 6907 - Psychological Safety And Leadership Behaviors](./converted-md/image-notes/IMG_6907-psychological-safety.md)
  - [IMG 6908 - 70/20/10 And Knowledge Creation](./converted-md/image-notes/IMG_6908-70-20-10-nonaka.md)
  - [IMG 6909 - Knowledge Home, AI Usage, And Mastery](./converted-md/image-notes/IMG_6909-ai-learning-amplifier.md)
  - [IMG 6910 - Storytelling, Work, And Resource Tension](./converted-md/image-notes/IMG_6910-storytelling-presentation.md)
  - [IMG 6911 - Personal Storytelling, Data, And Online Communication](./converted-md/image-notes/IMG_6911-data-culture-communication.md)
  - [IMG 6912 - Inclusion And Diversity](./converted-md/image-notes/IMG_6912-inclusion-diversity.md)

### Original Files

- Six official programme and classroom PDFs are stored in the private EMBA R2 bucket and served through protected `/api/emba/file/...` URLs after login.
- Handwritten-note images remain in `/emba/materials/2026-07/handwritten-notes/images/`.
- Each handwritten-note image now has its own Markdown mirror in `./converted-md/image-notes/`.
- Future PDF, PPT, Word, and slide files should be stored in private R2 while their searchable Markdown mirrors go into `./converted-md/`.

Source handling rule: keep source images and future original files intact. Markdown files are the searchable interpretation layer and should point back to the image, PDF, PPT, or Word source whenever possible.

### Course Notes

- Current first note is stored as a converted Markdown mirror because it was generated from handwritten-note images.

### Readings

- No reading summaries added yet.

### Case Studies

- No case studies added yet.

### Assignments

- No assignments added yet.

### Reflections

- [Leo's EMBA Thinking Journey - July 2026](./reflections/2026-07-leo-thinking-journey.md)
- [Personal Reflections - Source, Context, Codex Completion](./reflections/2026-07-questions-and-reflections-review.md)
- [Personal Reflection Evidence Ledger](./reflections/2026-07-personal-marker-original-extract.md)
- Monthly reflection is captured in the EMBA timeline, rebuilt from E001-E120 source evidence, and separated into T01-T19 cards for one-by-one review.

### Work Applications

- Apply leadership learning to meetings, coaching conversations, team learning routines, and AI-assisted preparation.

## 4. Key Topics

- Leadership as enabling others to think differently.
- Listening before advising.
- Coaching as helping the coachee repair or reframe their own problem.
- Reflection as a way to separate fact, interpretation, and perspective.
- AI as a learning amplifier, not a substitute for human judgment.
- Organizational learning as turning personal insight into shared practice.
- Inclusion as practical communication across culture, authority, feedback, and disagreement norms.

## 5. Key Concepts And Frameworks

- Levels of listening.
- Kolb experiential learning cycle.
- Problem-Based Learning.
- Leadership Development Trajectory support structure.
- Three voices: judgment, cynicism, fear.
- Psychological safety and accountability.
- 70/20/10 learning.
- Nonaka and Takeuchi knowledge creation.
- Culture Map.
- Storytelling with personal, authentic, audience-specific framing.

## 5A. Knowledge Map

| Cluster | What It Contains | Retrieval Keywords |
| --- | --- | --- |
| Programme operation | Onboarding, Canvas, assessment, BCP, LDT, electives, alumni network | need-to-knows, Canvas, resit, BCP, LDT, alumni |
| Learning design | Problem-Based Learning, Kolb cycle, active practice, future-self intention | PBL, experiential learning, reflection, future self |
| Self-observation | Autopilot, assumptions, blind spots, judgment, cynicism, fear | self-awareness, blind spot, three voices, reflection |
| Leadership environment | Safety, trust, focus, listening, accountability | psychological safety, better thinking, leadership posture |
| Coaching practice | Listening before advising, helping the coachee reframe the problem | coaching, listening levels, question, coachee |
| Learning loops | Repetition, reflection, 70/20/10, organizational learning | learning loop, 70/20/10, knowledge creation |
| AI practice | AI as mirror, assistant, pressure tester, and learning amplifier | AI, human judgment, roleplay, pressure-test |
| Inclusion and communication | Culture Map, storytelling, audience-specific communication | inclusion, diversity, culture, storytelling |

## 6. Personal Reflections

Leadership is not mainly about stronger answers. It is about creating the conditions for better thinking: safety, trust, listening, reflection, and courage to question assumptions.

The website presents each personal reflection in three layers only: Leo's original handwriting, the context in which it appeared, and Codex's completed argument. Classroom frameworks remain context rather than being counted as Leo's original reflections.

Detailed one-by-one review file:

- [July 2026 Leo Thinking Journey](./reflections/2026-07-leo-thinking-journey.md)
- [July 2026 Personal Reflections - Source, Context, Codex Completion](./reflections/2026-07-questions-and-reflections-review.md)
- [July 2026 Personal Reflection Evidence Ledger](./reflections/2026-07-personal-marker-original-extract.md)

## 7. Work Applications

- Before meetings: write the intended outcome and likely assumptions.
- During meetings: listen longer, summarize before responding, and ask one question that helps others think differently.
- After meetings: capture what happened, what I made it mean, and what else could be true.
- With AI: use it to summarize, roleplay, pressure-test, and find blind spots, while keeping human judgment in the lead.
- For teams: turn useful insight into shared language, meeting rhythm, checklist, or story.

## 8. Possible LinkedIn Or Blog Ideas

- Leadership is not giving better answers. It is building better thinking rooms.
- AI should raise the standard of learning, not replace learning.
- Blind spots are not shame. They are input.
- Listening is a leadership technology.

## 9. Related Theme Pages

- [Leadership](../../themes/leadership.md)
- [AI and Digital Transformation](../../themes/ai-and-digital-transformation.md)
- [Decision Making](../../themes/decision-making.md)
- [Personal Growth](../../themes/personal-growth.md)

## 10. Tags And Keywords

Tags: leadership, learning, coaching, AI, inclusion, organizational-learning.

Keywords: Lead in Learning, memory making, listening, psychological safety, 70/20/10, Nonaka Takeuchi, Culture Map, storytelling.

## 11. RAG And Search Notes

Use this monthly index as the entry point for broad July questions. Use the official programme mirror for the July 1-3 schedule. Use the five classroom-deck mirrors for LDT, team-building, PBL and welcome context, alumni, assessment, BCP, electives, and platform questions. Use the converted handwritten-note analysis for Leo's class-note interpretation or personal action plan. Use the personal marker extract for Leo:, I see, quoted lines, starred lines, question marks, source-image position, or original handwritten phrasing.

Useful Chinese search aliases: 领导力, 日程, 课程安排, 入学须知, 作业考试, 体验式学习, 问题导向学习, 校友网络, 倾听, 教练, 心理安全, 反思, 盲点, 人工智能, 组织学习, 包容性, 讲故事.
